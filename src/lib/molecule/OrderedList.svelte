<script lang="ts">
	...
	let mfolders = $derived(
		grouped.reverse() || [
			...
			{
				year: 2025,
				files: ['project-proposal.docx', 'final-report.pdf', 'presentation.pptx']
			},
			{
				year: 2024,
				files: [
			...
			{
				year: 2023,
				files: ['budget.xlsx', 'meeting-notes.txt']
			}
		]
	);
...
{#snippet mList()}
	<button
		...
		tabindex={pagination != 'none' ? undefined : -1}>Recent</button
	>
	...
	<button
		...
		tabindex={pagination != 'none' ? undefined : -1}>All</button
	>
{/snippet}
